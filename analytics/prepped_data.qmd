...
df_distances = get_distances(pl.read_parquet('analytics/prepped_data.parquet'))
```
...
df_model = (
    ...
    .select(
        [
            ...
            "delta_y",
            "velocity_alignment",
            "relative_speed",  # Add this to the final selection
        ]
    )
    .drop_nulls()
    .drop_nans()
...
def create_plot_df(df, play):
    ...
    if isinstance(df, str):
        return (
            pl.scan_parquet("analytics/prepped_data.parquet")
...
def create_player_df(df_preds, player_name):
    return df_preds.filter(pl.col("player_name") == player_name)
```

## Mike Evans

```{python}
df = 'analytics/prepped_data.parquet'
